...
public let kWeiboOAuthError = "kWeiboOAuthError"
...
extension MonkeyKing {
    ...
    public class func handleOpenURL(_ URL: Foundation.URL) -> Bool {
    ...
    public class func shareMessage(_ message: Message, completionHandler: @escaping SharedCompletionHandler) {
        ...
        case .qq(let type):
            ...
            if let media = type.info.media {
                ...
                func handleNewsWithURL(_ URL: Foundation.URL, mediaType: String?) {
                ...
                sharedMonkeyKing.request(URLString, method: .POST, parameters: parameters) { (responseData, HTTPResponse, error) -> Void in
                    if let JSON = responseData, let _ = JSON["idstr"] as? String {
                        ...
                        if let code = (HTTPResponse as? HTTPURLResponse)?.statusCode {
                            if code == 403 {
                                NotificationCenter.default.post(name: Notification.Name(rawValue: kWeiboOAuthError), object: nil)
                            }
                        }
...
extension MonkeyKing {
    ...
    public class func OAuth(_ platform: SupportedPlatform, scope: String? = nil, completionHandler: @escaping OAuthCompletionHandler) {
...
extension MonkeyKing: WKNavigationDelegate {
    ...
    public func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
        ...
        // QQ Web OAuth
        guard urlString.contains("&access_token=") && urlString.contains("qq.com") else {
            return
        }
        
        guard let fragment = URL.fragment?.characters.dropFirst(), let newURL = Foundation.URL(string: "http://qzs.qq.com/?\(String(fragment))") else {
            return
        }
    ...
    public func webView(_ webView: WKWebView, didReceiveServerRedirectForProvisionalNavigation navigation: WKNavigation!) {
        ...
        if urlString.hasPrefix("wx") {
            ...
            for case let .weibo(appID, appKey, redirectURL) in accountSet {
                if urlString.lowercased().hasPrefix(redirectURL) {
...
extension MonkeyKing {
    ...
    fileprivate class func createAlipayMessageDictionary(_ info: Info, appID: String) -> NSDictionary {
    ...
    fileprivate class func addWebViewByURLString(_ URLString: String) {
